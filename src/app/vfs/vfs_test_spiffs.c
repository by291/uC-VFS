...
LOG_MODULE_REGISTER(test_spiffs, LOG_LEVEL_DBG);
...
static void test_mkrmdir() {
  print_test_result("test_mkrmdir__mount", vfs_mount(&_test_vfs_mount) == 0);

  // spiffs doesn't support dir
  print_test_result("test_mkrmdir__mkdir",
                    vfs_mkdir(MNT_PATH "/" DIR_NAME, 0) == -ENOTSUP);
  print_test_result("test_mkrmdir__rmdir",
                    vfs_rmdir(MNT_PATH "/" DIR_NAME) == -ENOTSUP);

  print_test_result("test_mkrmdir__unmount",
                    vfs_umount(&_test_vfs_mount, false) == 0);
}
...
void test_vfs_spiffs() {
  print_test_banner("SPIFFS VFS Tests");
  spiffs_vfs_desc_init(&spiffs_desc);
