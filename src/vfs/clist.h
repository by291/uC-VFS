 ...
 * clist_is_empty()     | O(1)    | returns true if the list contains no
 * elements clist_exactly_one()  | O(1)    | returns true if the list contains
 * one element clist_more_than_one()| O(1)    | returns true if the list
 * contains more than one element
 *
 * clist can be used as a traditional list, a queue (FIFO) and a stack (LIFO)
 * using fast O(1) operations.
 *
 ...
 *
 * To use clist as a queue, use clist_rpush() for adding elements and
 * clist_lpop() for removal. Using clist_lpush() and clist_rpop() is inefficient
 * due to clist_rpop()'s O(n) runtime.
 *
 ...
 * @returns         NULL if node is not a list member
 */
static inline clist_node_t *clist_remove(clist_node_t *list,
                                         clist_node_t *node) {
  if (list->next) {
